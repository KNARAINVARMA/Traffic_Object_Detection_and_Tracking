"""
Roboflow detection + tracking on DJI_0009_first30s.MP4
Produces:
  - roboflow_DJI_0009_tracked.mp4   (stabilized boxes, same visual style)
  - roboflow_DJI_0009_tracks.csv    (for safety analysis)
"""
...
SOURCE    = "/Users/suyashagarwal/Desktop/btp/DJI_0009_first30s.MP4"
OUT_VIDEO = "/Users/suyashagarwal/Desktop/btp/traffic_tracking/outputs/video/roboflow_DJI_0009_tracked.mp4"
OUT_CSV   = "/Users/suyashagarwal/Desktop/btp/traffic_tracking/outputs/csv/roboflow_DJI_0009_tracks.csv"
SCALE_MPX = 0.05
CONF      = 0.3
IOU       = 0.7
SMOOTH_W  = 7   # moving-average window for box coordinates
...
class BoxSmoother:
    """Per-track moving average over (x1, y1, x2, y2)."""
    ...
    def update(self, tid: int, x1: float, y1: float, x2: float, y2: float
        ...
        return tuple(arr.mean(axis=0))   # (x1, y1, x2, y2)
...
with sv.VideoSink(OUT_VIDEO, video_info) as sink:
    for frame in tqdm(frame_generator, total=video_info.total_frames, desc="Roboflow"):
        ...
        if detections.tracker_id is not None and len(detections.tracker_id) > 0:
            ...
            for k, tid in enumerate(detections.tracker_id):
                sx1, sy1, sx2, sy2 = smoother.update(
                    int(tid), *detections.xyxy[k]
                )
                smoothed_xyxy[k] = [sx1, sy1, sx2, sy2]

            # Build CSV rows using smoothed coords
            for k, tid in enumerate(detections.tracker_id):
